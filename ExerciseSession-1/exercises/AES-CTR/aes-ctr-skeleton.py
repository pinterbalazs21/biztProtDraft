...
if operation == 'encode': 
    ...
    # generate a 64-bit (8-byte) random nonce to be used as a prefix of the counter
    nonce = Random.get_random_bytes(8)

    # create a counter object, set the nonce as its prefix and set the initial counter value to 0 
    ctr = Counter.new(___, prefix=___, initial_value=___)

    # create an AES-CTR cipher object
    key = keystring.___('ascii')
    cipher = AES.new(___, ___, counter=___)
